...
public class LanguageManager : MonoBehaviour
{
    ...
    private readonly string jsonFilePath = @"C:\KidingKiding_App\Language.json";
    ...
    public void TriggerTextEvent()
    {
        ...
        foreach (var item in m_LocalizableTexts)
        {
            ...
            string currentKeyCandidate = item.m_TextComponent.text;
            ...
            foreach (var tableItem in item.m_LocalizedAssetTables)
            {
                ...
                if (tableOp.Result is StringTable stringTable)
                {
                    // 🔑 Text 내용을 "Key"로 직접 매칭
                    var entry = stringTable.GetEntry(currentKeyCandidate);
                    if (entry != null)
                    {
                        ...
                        lse.StringReference = new LocalizedString
                        {
                            ...
                            TableEntryReference = entry.Key // 여기 Key는 한글로 작성 가능
                        };
                        lse.RefreshString();
                        keyFound = true;

                        Debug.Log($"[LanguageManager] Key \"{entry.Key}\" applied on {item.m_TextComponent.name}");
                        break;
                    }
                }
    ...
    private void CollectLocalizedTexts()
    {
        ...
        var found = FindObjectsOfType<LocalizeStringEvent>(true);
